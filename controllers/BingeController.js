...
const uri = `mongodb://${process.env.USERNAME}:${process.env.PASSWORD}@${process.env.CLUSTER_URL}:27017/?authMechanism=${process.env.AUTH_MECHANISM}`

exports.test = async function (req, res) {
    console.log({ req, res })
    try {
        apiResponse.successResponse(res, "Success.", await apiCall.axios(apiURL.testURL(req.body)))
    } catch (error) {
...
exports.getVideos = async function (req, res) {
    ...
    try {
        ...
        let mediaAvailable = await client
            .db("bingefeast")
            .collection("media")
...
exports.getOTTStreams = async function (req, res) {
    ...
    try {
        ...
        if (dataFromDB === null) {
            ...
            let dbCount = await db
                .collection("counters")
                .updateOne({ counterName: "utelly" }, { $inc: { counts: +1 } })
            console.log(
                `docs matched QRY:${dbCount.matchedCount}, doc updated:${dbCount.modifiedCount}`
            )
